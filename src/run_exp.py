import os
import json
import argparse
from typing import List

import torch
from torch.utils.data import DataLoader

from datasets import Dataset, DatasetDict
from transformers import (
    AutoTokenizer,
    AutoModelForSeq2SeqLM,
    DataCollatorForSeq2Seq,
)
from peft import LoraConfig, get_peft_model
import sacrebleu
from tqdm.auto import tqdm


# ---------- Simple SARI implementation (no easse) ----------

def _get_ngrams(tokens: List[str], n: int):
    return [" ".join(tokens[i:i+n]) for i in range(len(tokens)-n+1)]

def _sari_sentence(src: str, cand: str, refs: List[str], max_n: int = 4) -> float:
    src_toks = src.split()
    cand_toks = cand.split()
    refs_toks = [r.split() for r in refs]

    F_add_total = F_keep_total = F_del_total = 0.0
    n_count = 0

    for n in range(1, max_n + 1):
        n_count += 1
        src_ngrams = set(_get_ngrams(src_toks, n))
        cand_ngrams = set(_get_ngrams(cand_toks, n))
        refs_ngrams_list = [set(_get_ngrams(rt, n)) for rt in refs_toks]

        # Added
        cand_add = cand_ngrams - src_ngrams
        refs_add = set().union(*[(r - src_ngrams) for r in refs_ngrams_list]) if refs_ngrams_list else set()
        overlap_add = cand_add & refs_add
        P_add = len(overlap_add) / len(cand_add) if cand_add else 0.0
        R_add = len(overlap_add) / len(refs_add) if refs_add else 0.0
        F_add = 2 * P_add * R_add / (P_add + R_add) if (P_add + R_add) > 0 else 0.0

        # Kept
        cand_keep = cand_ngrams & src_ngrams
        refs_keep = set().union(*[(r & src_ngrams) for r in refs_ngrams_list]) if refs_ngrams_list else set()
        overlap_keep = cand_keep & refs_keep
        P_keep = len(overlap_keep) / len(cand_keep) if cand_keep else 0.0
        R_keep = len(overlap_keep) / len(refs_keep) if refs_keep else 0.0
        F_keep = 2 * P_keep * R_keep / (P_keep + R_keep) if (P_keep + R_keep) > 0 else 0.0

        # Deleted
        src_del = src_ngrams - cand_ngrams
        refs_del = set().union(*[(src_ngrams - r) for r in refs_ngrams_list]) if refs_ngrams_list else set()
        overlap_del = src_del & refs_del
        P_del = len(overlap_del) / len(src_del) if src_del else 0.0
        R_del = len(overlap_del) / len(refs_del) if refs_del else 0.0
        F_del = 2 * P_del * R_del / (P_del + R_del) if (P_del + R_del) > 0 else 0.0

        F_add_total += F_add
        F_keep_total += F_keep
        F_del_total += F_del

    F_add_avg = F_add_total / n_count
    F_keep_avg = F_keep_total / n_count
    F_del_avg = F_del_total / n_count

    return (F_add_avg + F_keep_avg + F_del_avg) / 3.0

def sari_corpus(sources: List[str], candidates: List[str], references: List[List[str]]) -> float:
    scores = [
        _sari_sentence(s, c, rs)
        for s, c, rs in zip(sources, candidates, references)
    ]
    return sum(scores) / len(scores)


# ---------- Load WikiLarge ----------

def read_parallel(src_path, dst_path, filter_short=True, filter_ratio=True):
    with open(src_path, "r", encoding="utf-8") as f_src, \
         open(dst_path, "r", encoding="utf-8") as f_dst:
        src_lines = [line.strip() for line in f_src.readlines()]
        dst_lines = [line.strip() for line in f_dst.readlines()]

    assert len(src_lines) == len(dst_lines)
    src_out, dst_out = [], []

    for s, d in zip(src_lines, dst_lines):
        if not s or not d:
            continue
        if filter_short and (len(s.split()) < 5 or len(d.split()) < 3):
            continue
        if filter_ratio and len(d.split()) >= len(s.split()):
            continue
        ratio = len(d.split()) / len(s.split())
        if filter_ratio and ratio >= 1:
            continue
        src_out.append(s)
        dst_out.append(d)

    return {"source": src_out, "target": dst_out}

def load_wikilarge(data_dir):
    train = read_parallel(
        os.path.join(data_dir, "wiki.full.aner.train.src"),
        os.path.join(data_dir, "wiki.full.aner.train.dst"),
        filter_short=True,
        filter_ratio=True
    )
    valid = read_parallel(
        os.path.join(data_dir, "wiki.full.aner.valid.src"),
        os.path.join(data_dir, "wiki.full.aner.valid.dst"),
        filter_short=True,
        filter_ratio=True
    )
    test = read_parallel(
        os.path.join(data_dir, "wiki.full.aner.test.src"),
        os.path.join(data_dir, "wiki.full.aner.test.dst"),
        filter_short=False,
        filter_ratio=False
    )

    ds_train = Dataset.from_dict(train)
    ds_valid = Dataset.from_dict(valid)
    ds_test = Dataset.from_dict(test)

    return DatasetDict(train=ds_train, validation=ds_valid, test=ds_test)


# ---------- Tokenizer + model preprocessing ----------

def preprocess(examples, tokenizer, max_source_len, max_target_len):
    model_inputs = tokenizer(
        examples["source"],
        max_length=max_source_len,
        truncation=True,
    )
    labels = tokenizer(
        text_target=examples["target"],
        max_length=max_target_len,
        truncation=True
    )
    model_inputs["labels"] = labels["input_ids"]
    return model_inputs


# Differentiable proxy loss (soft embeddings)
def soft_embedding_proxy_loss(
    logits: torch.Tensor,          # (B, T, V)
    labels: torch.Tensor,          # (B, T)
    embedding_weight: torch.Tensor,# (V, D)
    pad_id: int
) -> torch.Tensor:
    """
    Differentiable semantic proxy:
    - expected embedding under model distribution vs gold embedding
    - cosine distance between mean-pooled sentence vectors
    """
    # logits -> probs
    probs = torch.softmax(logits, dim=-1)  # (B, T, V)

    # expected embedding for each position: (B,T,V) @ (V,D) -> (B,T,D)
    pred_emb = probs @ embedding_weight

    # labels may contain -100 (ignore); replace for embedding lookup
    labels_fixed = labels.clone()
    labels_fixed[labels_fixed == -100] = pad_id

    gold_emb = embedding_weight[labels_fixed]  # (B,T,D)

    # mask out padding / ignored positions based on original labels
    mask = (labels != -100) & (labels_fixed != pad_id)  # (B,T)
    mask_f = mask.float().unsqueeze(-1)                 # (B,T,1)

    # mean-pool into sentence vectors
    pred_sum = (pred_emb * mask_f).sum(dim=1)  # (B,D)
    gold_sum = (gold_emb * mask_f).sum(dim=1)  # (B,D)
    denom = mask_f.sum(dim=1).clamp(min=1.0)   # (B,1)

    pred_vec = pred_sum / denom
    gold_vec = gold_sum / denom

    # cosine similarity
    pred_norm = torch.nn.functional.normalize(pred_vec, dim=-1)
    gold_norm = torch.nn.functional.normalize(gold_vec, dim=-1)
    cos = (pred_norm * gold_norm).sum(dim=-1)  # (B,)

    return 1.0 - cos.mean()


# ---------- Model building (optional LoRA) ----------

def maybe_add_lora(model):
    lora_config = LoraConfig(
        r=16,
        lora_alpha=32,
        target_modules=["q", "k", "v", "o"],
        lora_dropout=0.05,
        bias="none",
        task_type="SEQ_2_SEQ_LM",
    )
    model = get_peft_model(model, lora_config)
    model.print_trainable_parameters()
    return model


# ---------- Unified training loop ----------

def train_model(
    model,
    tokenizer,
    tokenized_datasets,
    data_collator,
    args,
):
    if args.train_type == "none":
        print("No training needed!")
        return model

    if args.train_type == "lora":
        print("Wrapping model with LoRA...")
        model = maybe_add_lora(model)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Device:", device)
    model.to(device)

    train_dataset_tok = tokenized_datasets["train"]
    if args.max_train_samples is not None and args.max_train_samples < len(train_dataset_tok):
        train_dataset_tok = train_dataset_tok.select(range(args.max_train_samples))

    train_loader = DataLoader(
        train_dataset_tok,
        batch_size=args.train_batch_size,
        shuffle=True,
        collate_fn=data_collator,
    )

    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr)

    # cache embedding matrix for proxy loss
    embedding_weight = model.get_input_embeddings().weight  # (V, D)

    print(f"Start {args.train_type} training on {len(train_dataset_tok)} examples")
    model.train()

    for epoch in range(args.num_epochs):
        epoch_bar = tqdm(train_loader, desc=f"Epoch {epoch+1}", leave=False)
        for batch in epoch_bar:
            batch = {k: v.to(device) for k, v in batch.items()}

            outputs = model(**batch)
            ce_loss = outputs.loss
            loss = ce_loss

            log_dict = {"loss": ce_loss.item(), "ce_loss": ce_loss.item()}

            # use differentiable proxy loss
            if args.use_proxy_loss:
                proxy = soft_embedding_proxy_loss(
                    logits=outputs.logits,
                    labels=batch["labels"],
                    embedding_weight=embedding_weight,
                    pad_id=tokenizer.pad_token_id
                )
                loss = ce_loss + args.lambda1 * proxy
                log_dict.update({
                    "loss": loss.item(),
                    "proxy_loss": proxy.item()
                })

            loss.backward()
            optimizer.step()
            optimizer.zero_grad()

            epoch_bar.set_postfix(log_dict)

    return model


# ---------- Evaluation: BLEU + SARI ----------

def evaluate(model, tokenizer, raw_datasets, tokenized_datasets, data_collator, args):
    print("Evaluating on test set (BLEU + SARI)...")
    test_dataset_tok = tokenized_datasets["test"]
    test_raw = raw_datasets["test"]

    test_loader = DataLoader(
        test_dataset_tok,
        batch_size=args.eval_batch_size,
        shuffle=False,
        collate_fn=data_collator,
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Device:", device)
    model.to(device)

    pred_texts = []
    model.eval()
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Generating", leave=False):
            batch = {
                k: v.to(device)
                for k, v in batch.items()
                if k in ["input_ids", "attention_mask"]
            }
            outputs = model.generate(
                **batch,
                max_length=args.max_target_length,
                num_beams=args.num_beams,
            )
            decoded = tokenizer.batch_decode(outputs, skip_special_tokens=True)
            pred_texts.extend(decoded)

    test_src = test_raw["source"][: len(pred_texts)]
    test_ref = test_raw["target"][: len(pred_texts)]

    bleu = sacrebleu.corpus_bleu(pred_texts, [test_ref]).score
    sari_raw = sari_corpus(test_src, pred_texts, [[r] for r in test_ref])
    sari = sari_raw * 100.0

    print(f"Test BLEU: {bleu:.2f}")
    print(f"Test SARI: {sari:.2f}")

    outputs = {
        "test_src": test_src,
        "test_ref": test_ref,
        "pred_texts": pred_texts
    }
    return bleu, sari, outputs


# ---------- Save result ----------

def save_result(model, tokenizer, bleu, sari, outputs, output_dir):
    os.makedirs(output_dir, exist_ok=True)

    test_src = outputs["test_src"]
    test_ref = outputs["test_ref"]
    pred_texts = outputs["pred_texts"]

    model.save_pretrained(output_dir)
    tokenizer.save_pretrained(output_dir)

    metrics = {"bleu": float(bleu), "sari": float(sari)}
    with open(os.path.join(output_dir, "metrics.json"), "w") as f:
        json.dump(metrics, f, indent=2)

    pred_path = os.path.join(output_dir, "predictions.tsv")
    with open(pred_path, "w", encoding="utf-8") as f:
        f.write("src\tref\tpred\n")
        for s, r, p in zip(test_src, test_ref, pred_texts):
            f.write(f"{s.replace('\t',' ')}\t{r.replace('\t',' ')}\t{p.replace('\t',' ')}\n")

    print("Saved model and outputs to:", output_dir)


# ---------- Argparse + Main ----------

def parse_args():
    p = argparse.ArgumentParser("T5 simplification training/eval with proxy semantic loss")

    p.add_argument("--data_dir", type=str, default="../data/data-simplification/wikilarge")
    p.add_argument("--model_name", type=str, default="t5-base")
    p.add_argument("--train_type", type=str, default="none",
                   choices=["none", "fine-tuning", "lora"])

    p.add_argument("--use_proxy_loss", action="store_true",
                   help="Enable differentiable soft-embedding semantic proxy loss.")
    p.add_argument("--lambda1", type=float, default=0.5,
                   help="Weight for proxy loss when enabled.")

    p.add_argument("--num_epochs", type=int, default=1)
    p.add_argument("--train_batch_size", type=int, default=32)
    p.add_argument("--eval_batch_size", type=int, default=32)
    p.add_argument("--lr", type=float, default=5e-5)

    p.add_argument("--max_source_length", type=int, default=128)
    p.add_argument("--max_target_length", type=int, default=128)
    p.add_argument("--max_train_samples", type=int, default=None)

    p.add_argument("--num_beams", type=int, default=4)

    p.add_argument("--output_root", type=str, default="../results")
    p.add_argument("--run_tag", type=str, default=None,
                   help="Optional extra tag for output dir name")

    return p.parse_args()


if __name__ == "__main__":
    args = parse_args()

    raw_datasets = load_wikilarge(args.data_dir)

    print(f"Loading model: {args.model_name}")
    tokenizer = AutoTokenizer.from_pretrained(args.model_name)
    model = AutoModelForSeq2SeqLM.from_pretrained(args.model_name)
    data_collator = DataCollatorForSeq2Seq(tokenizer, model=model)

    tokenized_datasets = raw_datasets.map(
        preprocess,
        fn_kwargs={
            "tokenizer": tokenizer,
            "max_source_len": args.max_source_length,
            "max_target_len": args.max_target_length,
        },
        batched=True,
        remove_columns=["source", "target"],
    )
    print(tokenized_datasets)

    model = train_model(
        model,
        tokenizer,
        tokenized_datasets,
        data_collator,
        args,
    )

    bleu, sari, outputs = evaluate(
        model, tokenizer, raw_datasets, tokenized_datasets, data_collator, args
    )

    base_tag = "proxy" if args.use_proxy_loss else "baseline"
    parts = [args.model_name.replace("-", "_"),
             args.train_type.replace("-", "_"),
             base_tag]

    if args.use_proxy_loss:
        lam_str = f"lam{args.lambda1:g}".replace(".", "p")  # e.g., 0.5 -> lam0p5
        parts.append(lam_str)

    if args.run_tag:
        parts.append(args.run_tag)

    output_dir = os.path.join(args.output_root, "_".join(parts))
    save_result(model, tokenizer, bleu, sari, outputs, output_dir)

